...
type Lexer struct {
	...
	position     int
	readPosition int
	ch           byte // ASCII support only (if UNICODE we would use rune)
}
...
// Returns the token associated with the current character and reads the next character
